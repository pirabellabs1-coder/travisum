/**
 * Serveur Node unique pour l'hébergement Plesk (Ubuntu, nginx en façade).
 *
 * Sert le site statique (dossier `out/` généré par `npm run build`), expose les
 * deux endpoints dynamiques `/api/chat` et `/api/rdv`, et applique les
 * redirections 301/308 définies dans `vercel.json`. Une seule application à
 * lancer dans Plesk (Node.js 20 recommandé).
 *
 * Démarrage :  npm install && npm run build && node server.mjs
 * Variables d'environnement (Plesk > Node.js > Variables) :
 *   OPENROUTER_API_KEY   assistant IA (sans elle : réponse de repli)
 *   CHAT_MODEL           optionnel (défaut anthropic/claude-haiku-4.5)
 *   RESEND_API_KEY       envoi e-mail des RDV (sans elle : repli mailto)
 *   RDV_TO, RDV_FROM     optionnels (destinataire / expéditeur des e-mails)
 *   PORT                 port d'écoute (défaut 3000 ; Plesk le fournit)
 */
...
} catch { /* pas de fichier : on continue sans redirections */ }
...
app.all(["/api/rdv", "/api/rdv/"], async (req, res) => {
